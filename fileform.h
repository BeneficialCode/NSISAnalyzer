...
#define NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
...
#define MAX_ENTRY_OFFSETS 8

#define FIRST_INT_TYPE INT64
#ifdef _MSC_VER // old mingw32 doesn't have i64 support, and a typecast doesn't work either
#  define FIRST_INT_FLAG 0x8000000000000000i64
#  define FIRST_INT_MASK 0x7fffffffffffffffi64
#else
#  define FIRST_INT_FLAG 0x8000000000000000LL
#  define FIRST_INT_MASK 0x7fffffffffffffffLL
#endif
#define FIRST_INT_MAXDATA FIRST_INT_MASK
#define FIRST_INT_FIX_ENDIAN FIX_ENDIAN_INT64
#define FIRST_INT_SIZEOF ((FIRST_INT_TYPE)sizeof(FIRST_INT_TYPE))
...
#define FH_FLAGS_EXTERNAL_FILE_SUPPORT 64
#define FH_FLAGS_HAS_EXTERNAL_FILE 128
#define FH_FLAGS_IS_STUB_INSTALLER 256
...
typedef struct
{
	...
	// this specifies the length of the datablock
	// nsisbi versions before 3.04.1 stored the external flag in the top bit of the high part
	int datablock_lowpart;
	int datablock_highpart;
#endif
} firstheader;
...
#define GetCompressedDataFromDataBlock(offset, hFileOut, crc) _dodecomp(DATALOC_DATABLOCK,offset,hFileOut,NULL,0,crc)
#define GetCompressedDataFromDataBlockToMemory(offset, out, out_len, crc) _dodecomp(DATALOC_DATABLOCK,offset,NULL,out,out_len, crc)
#define GetCompressedDataFromExeHead(offset, hFileOut, crc) _dodecomp(DATALOC_EXEHEAD,offset,hFileOut,NULL,0,crc)
#define GetCompressedDataFromExeHeadToMemory(offset, out, out_len, crc) _dodecomp(DATALOC_EXEHEAD,offset,NULL,out,out_len,crc)
...
extern firstheader g_firstheader;
