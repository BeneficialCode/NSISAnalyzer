...
TCHAR *  GetNSISString(TCHAR *outbuf, int strtab);
...
HKEY  GetRegKeyAndSAM(HKEY hKey, REGSAM*pRS);
LONG  RegKeyOpen(HKEY hBase, LPCTSTR SubKey, REGSAM RS, HKEY*phKey);
LONG  RegKeyCreate(HKEY hBase, LPCTSTR SubKey, REGSAM RS, HKEY*phKey);
void  myRegGetStr(HKEY root, const TCHAR *sub, const TCHAR *name, TCHAR *out, UINT altview);
...
HRESULT  UTF16LEBOM(HANDLE h, INT_PTR ForWrite);

//BOOL NSISCALL my_SetWindowText(HWND hWnd, const TCHAR *val);
#define my_SetWindowText SetWindowText
BOOL  my_SetDialogItemText(HWND dlg, UINT idx, const TCHAR *val);
...
int  my_GetDialogItemText(UINT idx, TCHAR *val);
...
BOOL  UserIsAdminGrpMember(); // Does not check integrity level, returns true if the process has a non-deny administrators group ACE in the token
DWORD  CreateRestrictedDirectory(LPCTSTR path);
DWORD  CreateNormalDirectory(LPCTSTR path);

HANDLE  myCreateProcess(TCHAR *cmd);
BOOL  myShellExecuteEx(SHELLEXECUTEINFO*pSEI);
int  my_MessageBox(const TCHAR *text, UINT type);
...
void  validate_filename(TCHAR *fn);
...
DWORD  remove_ro_attr(LPCTSTR file);
...
void  MessageLoop(UINT uCheckedMsg);
